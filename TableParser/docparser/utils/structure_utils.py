...
def merge_annotation_lists_util(base_list, new_list, new_list_page_nr):
    base_document_root_id = None
    base_document_meta_id = None
    for ann in base_list:
        if ann['category'] == 'document':
            base_document_root_id = ann['id']
        if ann['category'] == 'meta':
            base_document_meta_id = ann['id']
    max_id_base = max(x['id'] for x in base_list) + 1


    adapted_new_list = []
    for ann in new_list:
        if ann['category'] == 'document':
            new_document_root_id = ann['id']
        elif ann['category'] == 'meta':
            new_document_meta_id = ann['id']
        elif ann['category'] == 'unk':
            continue
        else:
            if 'page' in ann:
                ann['page'] = new_list_page_nr
            adapted_new_list.append(ann)

    for ann in adapted_new_list:
        if ann['parent'] == new_document_root_id:
            ann['parent'] = base_document_root_id
        elif ann['parent'] == new_document_meta_id:
            ann['parent'] = base_document_meta_id
        elif ann['parent'] is not None:
            ann['parent'] += max_id_base
        ann['id'] +=  max_id_base

            
    merged_anns_list = base_list + adapted_new_list
